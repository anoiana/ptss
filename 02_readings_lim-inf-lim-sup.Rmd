...
d = cbind(n,s)
r = runif(100,-1,0)
plot(d[d[,2]<0,], pch = 16, ylim = c(-6,0), cex = 1.2, ylab = "sequence (c)")
...
r = runif(100,0,1)
plot(d[d[,2]>0,], pch = 16, cex = 1.2, ylab = "sequence (c)", ylim = c(0,3.5))
points(r, pch = 16, col = "red")
abline(h = 1, col = "red")
```
...
```{r,  echo=FALSE, fig.cap="example for $\\lim\\sup$", fig.align='center'}
r = runif(500,-1,1)
plot(d, pch = 16, cex = 1.2, ylab = "sequence (c)")
points(r, pch = 16, col = "red")
abline(h = 1, col = "red")
abline(h = -1, col = "red")
```
...
$$
which has a general form in `r lb(eq2)`. The case of supremum can be explained with an analogous manner. 
