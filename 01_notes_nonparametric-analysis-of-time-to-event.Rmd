...
Each of $n$ patients has a counting process that registers the number of occurrences of an event of interest. We shall denote it as $N_i(t)$ for $i$th patient in the time interval $[0,t]$. The intensity process of $N_i(t)$ has the following form 
$$
...
More general, Markov chain can be used with a finite number of states in which the transition intensity $\alpha_{gh}(t)$ describes the instantaneous risk of transition from state $g$ to $h$. In other words, $\alpha_{gh}(t)dt$ is the conditional probability a patient who is in state $g$ just before time $t$ will make a transition to state $h$ in the small time interval $[t,t+dt)$, for further details refer to @aal2008. 

For example, we deem a situation where the event of interest is dead, but hazard rates are not identical across subjects (or subgroups). This is 
$$
\alpha_i(t) = \alpha(t)\mu_i(t),
(\#eq:eq9)
$$
where $\alpha(t)$ is termed relative mortality common to all individuals and $\mu_i(t)$ is hazard rate (mortality rate) at time $t$ that is from a standard population relative to subject $i$, e.g., sex, age, etc. The process $N(t)$ that counts the number of deaths in $[0,t]$ has intensity process of multiplicative form $\alpha(t)Y(t)$, where $Y(t) = \sum_{i=1}^nY_i(t)\mu_i(t)$. Thus, the Nelson-Aalen estimator for cumulative relative mortality $A(t) = \int_0^t\alpha(u)du$ is 
$$
\hat{A}(t) = \sum_{T_j \le t}\frac{1}{\sum_{i=1}^nY_i(T_j)\mu_i(T_j)}.
(\#eq:eq10)
$$

## Ties

So far we have restricted our calculation to no tie event times, which make more sense with continuous case but this assumption is unrealistic in practice.  
