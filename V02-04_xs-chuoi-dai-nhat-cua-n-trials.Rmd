...
\end{align}
$$
ta nhận thấy rằng vì `r lb(eq3)` *($z_{n+1} -z_n + qp^rz_{n-r} = 0$)* nên
$$
\begin{align}
&(z_{r+1} -z_{r}+qp^rz_0)\rho^{r+1}+ (z_{r+2} -z_{r+1}+qp^{r}z_1)\rho^{r+2} + (z_{r+3} -z_{r+2}+qp^{r}z_2)\rho^{r+3} + \dots + \\
...
$$
và thay $\rho=0$, ta sẽ có giá trị $z_n = 1 - y_n$, mà theo định nghĩa ban đầu thì $y_n$ chính là một chuỗi gồm ít nhất $r$ mặt hình xuất hiện liên tục trong $n$ lần tung đồng xu đầu tiên. Nói cách khác, hệ số phía trước $\rho^n$ chính là xác suất mà chuỗi gồm ít nhất $r$ mặt hình KHÔNG THỂ xuất hiện liên tục trong $n$ lần tung đồng xu đầu tiên. 
